...
import { isForm, isFormEntry } from "./util";
...
export class FormController {
    private _form: Form;
    private _value: any = {};
    private _entriesInitialized: boolean = false;
    ...
    lookupParent(path: string): FormEntry | null {
        console.log("Looking up parent of " + path);
        const node = this._registry.lookupNode(path);
        if (node && node.parent) {
            console.log("Got parent: " + node.parent.path);
            return node.parent;
        }
    ...
    lookupChildren(path: string): FormEntry[] {
        ...
        if (node && node.sortedChildren) {
            const children = [];
            for (let n of node.sortedChildren) {
                if (n.entry) {
    ...
    updateValue(path: string, value: any): void {
        ...
        if (!entry) {
            console.error(`Failed to update value: No entry found at path ${path}`);
            return;
        }
        if (!entry.isForm() && !entry.isParameter()) {
            console.error(`Failed to update value: Only forms or parameters can have values. Entry at path "${path}" is a ${entry.entryType.toLowerCase()}`);
            return;
        }
...
class FormControllerRegistry {
    ...
    register(parent: FormEntry, entry: FormEntry): string {
        ...
        if (parent.isForm() && !parent.isSubForm) {
            ...
            if (!parent.path) {
                throw new Error("Parent has no path. This may be the case if it has not been added to a form");
            }
        ...
        for (let i = 0; i < pathParts.length; i++) {
            ...
            if (!nextNode) {
                ...
                if (nextNode.entry) {
                    throw new Error(`Failed to register entry ${entry.path}: An entry with that path is already registered.`);
                }
                if (currNode.entry)
    ...
    forEach(callback: (entry: FormEntry) => void) {
        const walk = (node: EntryTreeNode) => {
            ...
            if (node.sortedChildren) {
                for (let child of node.sortedChildren) {
                    walk(child);
                }
            }
        };
