...
import { AbstractContainerFormEntry, FormEntryWithValue, FormEntryWithValueInit } from "./form-entry";
import { isForm } from "./util";

export interface FormInit extends FormEntryWithValueInit {}
 ...
 */
export class Form extends AbstractContainerFormEntry implements FormEntryWithValue {
    isSubForm: boolean;
    ...
    constructor(formOrInit: Form | FormInit, initOrUndefined?: FormInit) {
        isForm(formOrInit) ? super(formOrInit, initOrUndefined!) : super("_useThis", formOrInit);

        let init: FormInit;
        if (isForm(formOrInit)) {
