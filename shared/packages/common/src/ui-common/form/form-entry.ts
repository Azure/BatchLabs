...
import { findParentForm, isForm, isParameter, isSection } from "./util";
...
export abstract class AbstractFormEntry implements FormEntry {
    ...
    __setPathInternal(path: string) {
        if (this._path) {
            throw new Error("Cannot change an entry's path once it has been set");
        }
        this._path = path;
    }
}
 ...
 */
export abstract class AbstractContainerFormEntry extends AbstractFormEntry implements ContainerFormEntry {
    constructor(rootForm: Form | "_useThis", init: FormEntryInit) {
        super(rootForm, init);
    }
    ...
    getChild(pathOrId: string): FormEntry {
        ...
        if (this.isForm() && !this.isSubForm) {
            ...
            throw new Error("Parent entry has no path. Has it been added to a form?");
        } else {
    ...
    addChild(child: FormEntry): FormEntry {
        if (child.isForm() && !child.isSubForm) {
            throw new Error("Root forms may not be added as children to another form");
        }
        if (child.rootForm !== this.rootForm) {
            throw new Error("Parent and child form entries must share the same root form");
        }
